...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[url('/images/banner_backgroundlogin.webp')] bg-cover bg-center bg-slate-50 dark:bg-dark-bg relative overflow-hidden">
      {/* Background Effects */}
      ...
      <div className="max-w-md w-full relative z-10 px-4">
        ...
        <div className="bg-white dark:bg-dark-card border border-slate-200 dark:border-white/10 rounded-2xl shadow-xl p-8 backdrop-blur-sm">
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full flex justify-center items-center py-3 px-4 border border-transparent rounded-xl shadow-lg shadow-primary-500/20 text-sm font-bold text-white bg-[#a87a0c] hover:bg-[#8a5f0a] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#a87a0c] disabled:opacity-50 disabled:cursor-not-allowed transition-all active:scale-[0.98]"
            >
              {loading ? (
        ...
        <p className="mt-8 text-center text-xs text-slate-400 dark:text-slate-500">
          &copy; {new Date().getFullYear()} CRM 77 Marketing | Todos os direitos
          reservados CNPJ: 63.895.708/0001-78.
        </p>
      </div>
    </div>
  );
